...
public:
  ...
  void pub_odom_func(IMUST &xc)
  {
    ...
    br.sendTransform(tf::StampedTransform(transform, ct, "map", "odom"));
  }
...
public:
  ...
  VOXEL_SLAM(ros::NodeHandle &n)
  {
    ...
    if(feat.lidar_type == LIVOX)
      sub_pcl = n.subscribe<livox_ros_driver2::CustomMsg>(lid_topic, 1000, pcl_handler);
    else
    ...
    {
      // 目录已存在，删除后重新创建以覆盖旧文件
      printf("Directory exists, removing old files to allow overwrite...\n");
      string cmd = "rm -rf " + savepath + bagname + "/";
      int ret = system(cmd.c_str());
      cmd = "mkdir " + savepath + bagname + "/";
      ss = system(cmd.c_str());
    }
